...
import { problemIds } from '@/app/utils/problems';
...
const Topbar = ({ problemPage, pid }: TopbarProps) => {
  ...
  const handleProblemhange = (isForward: boolean) => {
    ...
    if (!isForward && nextProblemOrder === 0) {
      const firstProblemKey = problemIds.find(key => problems[key].order === 1);
      router.push(`/problems/${firstProblemKey}`);
    } else if (isForward && !nextProblemKey) {
      const lastProblemKey = problemIds.find(
        key => problems[key].order === problemIds.length
      );
        ...
        {problemPage && (
          <div className="flex items-center gap-4 flex-1 justify-center">
            <div
              ...
              onClick={() => handleProblemhange(false)}>
              <FaChevronLeft />
            </div>
            <Link
              href="/"
            ...
            <div
              className="flex items-center justify-center rounded bg-dark-fill-3 hover:bg-dark-fill-2 h-8 w-8 cursor-pointer"
              onClick={() => handleProblemhange(true)}>
              <FaChevronRight />
            </div>
          </div>
        )}
